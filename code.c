/*
 * lab05.c — Distributed WMA via Socket Programming (Extension of lab04)
 *
 * EXTENSION OVERVIEW:
 * -------------------
 * Beyond lab04's matrix scatter, each slave now:
 *   (1) Computes an Order-Q Weighted Moving Average (WMA) column-wise across
 *       its assigned row-slice of matrix X, producing a partial result vector p.
 *   (2) Sends that partial p back to the master (M1PR: Many-to-One Point Return).
 *
 * The master collects all partial p vectors, assembles the full p, and reports
 * total time_elapsed (distribution + collection). Each slave independently
 * reports ONLY its computation time_elapsed (after receiving, before sending).
 *
 * WMA DEFINITION (Order Q, column j):
 *   WMA(i, j) = ( Σ_{k=0}^{q-1} w[k] * X[i - q + k][j] ) / weight_sum
 *   for i = q, q+1, ..., m-1   (m = number of rows in the slave's submatrix)
 *
 * RMSE FOR COLUMN j (slave's partial result):
 *   p[j] = sqrt( Σ_{i=q}^{m-1} (X[i][j] - WMA(i,j))^2 ) / (m - q)
 *
 * SLIDING WINDOW OPTIMIZATION (row-wise, O(n^2) instead of O(n^3)):
 *   For a fixed column j, as the row index i advances from q to m-1:
 *     - The weighted sum shifts by removing the oldest row and adding the newest.
 *     - A precomputed weight-difference array avoids recomputing the full dot
 *       product at each step.
 *   See mse_wma() for the detailed derivation in comments.
 *
 * PROTOCOL SEQUENCE (per connection):
 *   Master → Slave : [rows][n_cols][submatrix rows][q][weight_sum][w array]
 *   Slave  → Master: [ack][p slice (n floats)]
 *
 * COMPILATION:
 *   gcc -o lab05 lab05.c -lpthread -lm
 *
 * USAGE (same as lab04, three inputs: n, p, s):
 *   config.txt format:
 *     MASTER <ip> <port>
 *     <num_slaves>
 *     SLAVE <ip> <port>
...
typedef struct ARGS {
    ...
    float **X;              // Pointer to the full shared matrix (read-only)

    int q;                  // WMA order
    float weight_sum;       // Sum of all weights in w
    float *w;               // Weight vector of length q (shared, read-only)

    float *p;               // FULL result vector p[n] (shared, write protected by mutex)
    pthread_mutex_t *mutex; // Mutex guarding writes into p
} args;
...
/*
 * worker — Thread function for the master.
 *
 * Each thread:
 *   1. Connects to its assigned slave.
 *   2. Sends: metadata (rows, n), submatrix rows, then WMA params (q, weight_sum, w).
 *   3. Receives "ack" from the slave.
 *   4. Receives the slave's computed partial p vector (n floats).
 *   5. Merges the received p into the shared p using mutex-protected accumulation.
 *
 * PROTOCOL ORDER (must match slave receive order exactly):
 *   → int   rows        (network byte order)
 *   → int   n_cols      (network byte order)
 *   → float submatrix[rows][n_cols]  (row by row)
 *   → int   q           (network byte order)
 *   → float weight_sum
 *   → float w[q]
 *   ← char  ack[3]      "ack"
 *   ← float p_partial[n]
...
void *worker(void *arg) {
    ...
    // htonl() ensures correct byte order across different architectures
    ...
    // Sends rows [start_row .. start_row + rows - 1] of the full matrix X
    for (int r = 0; r < a->rows; r++) {
        send_all(sock, a->X[a->start_row + r], sizeof(float) * a->n);
    }

    // Send WMA parameters: q, weight_sum, w[]
    // These are sent AFTER the submatrix to match the slave's receive order
    int net_q = htonl(a->q);
    ...
    // The slave computes RMSE for all n columns across its row slice.
    ...
    // strictly speaking they don't overlap, but we use the mutex anyway for
    // safety in case future changes introduce overlap.
    float *p_partial = malloc(sizeof(float) * a->n);
    if (!p_partial) { perror("malloc p_partial"); pthread_exit(NULL); }

    recv_all(sock, p_partial, sizeof(float) * a->n);

    // Write received partial p into shared p[] (mutex-protected)
...
// receive back the partial RMSE vectors. Records total time_elapsed from
// first thread launch to last join (full round-trip including communication)
void master(int n, int t, addr **slave_addresses, float **X,
    ...
    for (int i = 0; i < t; i++) {
        ...
        // +1 skips core 0 which is reserved for the main thread
        pthread_attr_t attr;
    ...
    // Wait for ALL threads to complete (all ACKs received and p[] assembled)
    for (int i = 0; i < t; i++) {
        pthread_join(threads[i], NULL);
    }

    // time_after: all slaves done, p[] fully assembled 
...
// Slave Function ---------------------------------------------------
// ------------------------------------------------------------------
// slave receives submatrix + WMA params, computes RMSE, returns p.
...
int main() {
    ...
    //   p = port this instance listens on
    ...
    // ---- SLAVE BRANCH ----
        ...
        // Read master IP from config file (stored for reference)
        FILE *file = fopen("config.txt", "r");
        if (!file) {
